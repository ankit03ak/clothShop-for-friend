...
const Navbar = () => {
  ...
  {/* WhatsApp Link */}
  <a
    href="https://wa.me/917405954946"
    target="_blank"
    rel="noopener noreferrer"
  >
    <FaWhatsapp className="cursor-pointer text-green-400 hover:text-green-600 transition-colors duration-300 text-2xl md:text-4xl" />
  </a>
</div>
  ...
  <div className="flex flex-wrap items-center justify-center space-x-4 mt-2 text-center">
    {/* Contact */}
    <span className="text-md font-bold bg-gradient-to-r from-green-400 to-blue-400 bg-clip-text text-transparent">
      📞 7405954946
    </span>
  ...
  {/* Status / Users Box */}
  <div className="flex items-center bg-white/10 backdrop-blur-md rounded px-3 py-1 md:px-4 md:py-2 border border-white/20 shadow-lg">
    <div className="relative flex items-center">
      ...
      <span className="ml-2 text-xs md:text-sm font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
        {isAdminRoute ? `Total Visits : ${totalVisitors}` : `Users : ${activeUsers}`}
      </span>
    </div>
  </div>
  ...
  {!user ? (
    <div className="flex flex-wrap gap-2">
      ...
      <Link 
        to="/signup" 
        className="px-3 py-1 md:px-4 md:py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-semibold rounded-full shadow-lg hover:shadow-xl hover:scale-105 border border-purple-400/20"
      >
        Signup
      </Link>
    </div>
